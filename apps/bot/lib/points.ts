...
export const tryToAssignRegularMemberRole = async (member: GuildMember) => {
  if (!env.REGULAR_MEMBER_ROLE_ID) return

  const lastSync = lastUserSync.get(member.id)
  if (lastSync && Date.now() - lastSync < USER_ROLE_SYNC_INTERVAL) return
    ...
    embeds: [
      {
        ...
        description:
          'You have been assigned the Regular Member role! This role is given to users who have contributed to the community by helping other users and participating on the server. Thank you for your contributions!',
      },
    ],
  })
}
